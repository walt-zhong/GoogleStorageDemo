...
class ImageProvider : ContentProvider() {
    ...
    override fun query(
        ...
        val result = MatrixCursor(resolveDocumentProjection(projection as Array<String>?))
        ...
        val maxIndex = (offset?.plus(limit!!) ?: -1).coerceAtMost(files.size)
        if (offset != null) {
        ...
        val bundle = queryArgs?.let { constructExtras(it, files) }
